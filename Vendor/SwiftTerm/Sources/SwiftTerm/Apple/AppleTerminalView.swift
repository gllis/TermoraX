...
struct ViewLineInfo {
    ...
    // CJK glyphs are drawn directly. Asking Core Text for glyphs from one
    // font and then drawing them with another produces .notdef white blocks.
    var eastAsianGlyphs: [EastAsianGlyph]
}

struct EastAsianGlyph {
...
extension TerminalView {
    ...
    func buildAttributedString (row: Int, line: BufferLine, cols: Int, prefix: String = "") -> ViewLineInfo
    {
        ...
        for col in 0..<cols {
            ...
            if characterNeedsFallbackFont(ch) {
                let runAttributes = getAttributes(ch.attribute, withUrl: ch.hasPayload) ?? [:]
                eastAsianGlyphs.append(
                    EastAsianGlyph(
                        character: ch.getCharacter(),
                        column: col,
                        font: eastAsianFont(
                            bold: ch.attribute.style.contains(.bold),
                            italic: ch.attribute.style.contains(.italic)
                        ),
                        color: runAttributes[.foregroundColor] as? TTColor ?? nativeForegroundColor
                    )
                )
                // Preserve one UTF-16 unit per terminal cell for selection and
                // background layout, but never let Core Text draw a fallback box.
                str.append(" ")
            } else {
    ...
    func eastAsianFont(bold: Bool, italic: Bool) -> TTFont {
        let size = fontSet.normal.pointSize
        #if os(macOS)
        let names = bold
            ? ["PingFangSC-Semibold", "PingFangSC-Medium", "HiraginoSansGB-W6", "PingFangSC-Regular"]
            : ["PingFangSC-Regular", "HiraginoSansGB-W3", "PingFangSC-Medium"]
        for name in names {
        ...
        return UIFont(name: name, size: size) ?? UIFont.systemFont(ofSize: size, weight: bold ? .semibold : .regular)
        #endif
    }
    ...
    func drawTerminalContents (dirtyRect: TTRect, context: CGContext, bufferOffset: Int)
    {
        ...
        for row in firstRow...lastRow {
            ...
            for run in CTLineGetGlyphRuns(ctline) as? [CTRun] ?? [] {
                ...
                // Position by attributed-string index (= terminal column). Using
                // `col + glyphIndex` treats every glyph as 1 column, so CJK/emoji
                // (2 cells) overlap the next character.
                ...
                for i in 0..<runGlyphsCount {
                    let cell = min(max(0, Int(stringIndices[i])), max(0, terminal.cols - 1))
                    positions[i] = CGPoint(
                ...
                if runGlyphsCount > 0 {
                    let last = Int(stringIndices[runGlyphsCount - 1])
                    col = min(terminal.cols, last + 1)
                }
            }

            // Draw East Asian glyphs with glyph IDs obtained from the exact
            // font used for drawing. Core Text's fallback runs can otherwise
            // return glyph IDs belonging to a hidden fallback face, and
            // CTFontDrawGlyphs renders those IDs as white .notdef blocks.
            for item in lineInfo.eastAsianGlyphs {
                let utf16 = Array(String(item.character).utf16)
                ...
                guard CTFontGetGlyphsForCharacters(
                    ...
                    &glyphs,
                    characters.count
                ), let glyph = glyphs.first, glyph != 0 else {
                    continue
                }
                var oneGlyph = glyph
                var position = CGPoint(
                    x: lineOrigin.x + cellDimension.width * CGFloat(item.column),
                    y: lineOrigin.y + yOffset
                )
                context.setFillColor(item.color.cgColor)
                CTFontDrawGlyphs(item.font, &oneGlyph, &position, 1, context)
            }
